...
export class Robutek<
    PinsType extends typeof PinsV1 | typeof PinsV2,
> extends DifferentialDrive {
    public readonly Pins: PinsType;
    public bq2589?: BQ2589;
    ...
    constructor(
        ...
        encTicks: number,
        reg: motor.RegParams,
        robutekConfig: RobutekConfig,
        private ledcConfig: RobutekLedcConfig,
    ) {
        const wheelCircumference = Math.PI * robutekConfig.wheelDiameter;
        ...
        const rightMotorPins: motor.MotorPins = {
            motA: pins.Motor2A,
            motB: pins.Motor2B,
            encA: pins.Enc2A,
            encB: pins.Enc2B,
        ...
        const leftMotorLedc: motor.LedcConfig = {
            timer: ledcConfig.timer,
            channelA: ledcConfig.channels[0],
            channelB: ledcConfig.channels[1],
        };
        const rightMotorLedc: motor.LedcConfig = {
            timer: ledcConfig.timer,
            channelA: ledcConfig.channels[2],
            channelB: ledcConfig.channels[3],
        };

        const leftMotor = new motor.Motor({
            pins: leftMotorPins,
            ledc: leftMotorLedc,
            encTicks: encTicks,
            reg,
        ...
        const rightMotor = new motor.Motor({
            pins: rightMotorPins,
            ledc: rightMotorLedc,
            encTicks: encTicks,
            reg,
        ...
        super(leftMotor, rightMotor, robutekConfig.robutekDiameter);
        this.stop();
...
export const defaultRobutekConfig: RobutekConfig = {
    robutekDiameter: 83, // mm
    wheelDiameter: 33.3, // mm
};

export function createRobutek(
    version: "V1",
    robutekConfig?: RobutekConfig,
    ledcConfig?: RobutekLedcConfig,
): Robutek<typeof PinsV1>;
export function createRobutek(
    version: "V2",
    robutekConfig?: RobutekConfig,
    ledcConfig?: RobutekLedcConfig,
): Robutek<typeof PinsV2>;
export function createRobutek(
    version: "V1" | "V2",
    robutekConfig: RobutekConfig = defaultRobutekConfig,
    ledcConfig: RobutekLedcConfig = defaultLedcConfig,
): Robutek<typeof PinsV1 | typeof PinsV2> {
    if (version === "V1") {
        return new Robutek(
            PinsV1,
            EncoderTicksV1,
            ...
            robutekConfig,
            ledcConfig,
        ) as Robutek<typeof PinsV1>;
    } else {
        const robutek = new Robutek(
            PinsV2,
            EncoderTicksV2,
            RegV2,
            robutekConfig,
            ledcConfig,
        ) as Robutek<typeof PinsV2>;
        I2C1.setup({ scl: robutek.Pins.SCL, sda: robutek.Pins.SDA });
        robutek.bq2589 = new BQ2589(I2C1);
        robutek.bq2589.startBatteryChecker((status) =>
            robutek.setStatusLED(status),
        );
        return robutek;
    }
}
