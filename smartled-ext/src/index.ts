...
export class SmartLedExt {
    ...
    constructor(smartLed: SmartLed, indexFrom?: number, indexTo?: number) {
        ...
        // nut public number for now set to infinity
        const count = Infinity; // smartLed.count;
        if (indexFrom !== undefined || indexTo !== undefined) {
            const from = indexFrom ?? 0;
            const to = indexTo ?? count - 1;
            if (from < 0 || to >= count || from > to) {
                throw new RangeError(
                    "Invalid indexFrom/indexTo range for SmartLedExt",
                );
            }
            this.indexFrom = from;
            this.indexTo = to;
            ...
            this.indexFrom = 0;
            this.indexTo = count - 1;
        }

        // console.log(`Leds ${this.indexFrom} to ${this.indexTo} mapped to SmartLed with ${count} leds`);
    }
