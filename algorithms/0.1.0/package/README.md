...
### Larger Example (ADC -> PWM with PID)

The following example builds on your ADC-to-PWM pattern and adds PID so LED brightness changes smoothly and controllably.
...
const PWM_RESOLUTION = 10;
const PWM_MIN = 0;
const PWM_MAX = (1 << PWM_RESOLUTION) - 1;
...
const clamp = (value: number, min: number, max: number) =>
    Math.min(Math.max(value, min, max), max);
